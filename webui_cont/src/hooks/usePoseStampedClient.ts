...
export function usePoseStampedClient({ ros, isRosConnected, ros3dViewer, customTFProvider, topic, fixedFrame, options = {} }: UsePoseStampedClientProps) {
  ...
  const visualizationGroupRef = useRef<Group | null>(null);
  const trailPointsRef = useRef<Vector3[]>([]);
  const trailLineRef = useRef<Line | null>(null);
  ...
  const createArrow = useCallback((length: number, width: number, c: string) => {
    ...
    const mat = new MeshLambertMaterial({ color: c });
    const shaftGeometry = new CylinderGeometry(width*0.3, width*0.3, length*0.8, 8);
    const shaft = new Mesh(shaftGeometry, mat); shaft.rotation.z = -Math.PI/2; shaft.position.set(length*0.4,0,0); group.add(shaft);
    const headGeometry = new ConeGeometry(width, length*0.2, 8) as unknown as BufferGeometry; // cast to satisfy minimal augmented typing
    const head = new Mesh(headGeometry, mat); head.rotation.z = -Math.PI/2; head.position.set(length*0.9,0,0); group.add(head);
    return group;
  }, []);
  const createAxes = useCallback((size: number) => new ROS3D.Axes({ lineSize: size }), []);
  const updateTrail = useCallback((position: Vector3) => {
    ...
    if (trailPointsRef.current.length > 1) {
      const geometry = new BufferGeometry().setFromPoints(trailPointsRef.current);
      const material = new LineBasicMaterial({ color: typeof color === 'string' ? color : '#00ff00', opacity:0.6, transparent:true });
      trailLineRef.current = new Line(geometry, material);
  ...
  useEffect(() => {
    ...
    if (!visualizationGroupRef.current) { visualizationGroupRef.current = new Group(); ros3dViewer.current.addObject(visualizationGroupRef.current); }
