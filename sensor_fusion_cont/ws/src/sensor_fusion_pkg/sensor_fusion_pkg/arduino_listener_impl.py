#!/usr/bin/env python3
# Implementation moved from top-level arduino_listener.py so it can be packaged.
from . import __init__  # noqa: F401
import os, time, math, threading, serial, yaml
import rclpy
...
class ArduinoImuNode(Node):
    ...
    def reader_loop(self):
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                ser = None; time.sleep(retry_delay)
    ...
    def parse_line(self, line: str):
        ...
        tag = parts[0].upper(); imu_msg = Imu()
        imu_msg.header.stamp = self.get_clock().now().to_msg(); imu_msg.header.frame_id = self.frame_id
        try:
            if tag == 'IMU' and len(parts) >= 11:
                qw,qx,qy,qz = map(float, parts[1:5]); ax,ay,az = map(float, parts[5:8]); gx,gy,gz = map(float, parts[8:11])
            elif tag == 'RAW' and len(parts) >= 13:
                ax,ay,az = map(float, parts[1:4]); gx,gy,gz = map(float, parts[4:7]); yawd, rolld, pitchd = map(float, parts[10:13])
                yaw = math.radians(yawd); roll = math.radians(rolld); pitch = math.radians(pitchd)
                qw,qx,qy,qz = self.quaternion_from_euler(roll, pitch, yaw)
            else:
                return None
        except ValueError:
            return None
        imu_msg.orientation.w = qw; imu_msg.orientation.x = qx; imu_msg.orientation.y = qy; imu_msg.orientation.z = qz
    ...
    def publish_diag(self):
        da = DiagnosticArray(); da.header.stamp = self.get_clock().now().to_msg(); st = DiagnosticStatus()
        ...
        st.values = [KeyValue(key='port', value=self.port), KeyValue(key='baud', value=str(self.baud)), KeyValue(key='last_line', value=self.last_line[:80]), KeyValue(key='port_ok', value=str(self.port_ok))]
        da.status.append(st); self.diag_pub.publish(da)
    def destroy_node(self):
        self.stop_event.set(); super().destroy_node()

def main():
    rclpy.init(); node = ArduinoImuNode()
    try: rclpy.spin(node)
